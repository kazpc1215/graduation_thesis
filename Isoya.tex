...
これは積分できて，
\begin{equation}
\dot{x}^2 + \dot{y}^2 + \dot{z}^2 = 2 U - C_J,
\end{equation}
...
\end{split}
\end{equation}
ここで，${\bm A}^{\bm T}, {\bm B}^{\bm T}$ は ${\bm A}, {\bm B}$ の転置行列を表す．${\bm A}, {\bm B}$ はどちらも直交行列なので，逆行列は単純に自身の転置行列である．回転行列を作用させても距離は常に変わらないので (言い換えると，直交行列の行列式は$\pm 1$なので)，$x^2 + y^2 + z^2 = \xi^2 + \eta^2 + \zeta^2$ である．このことは，(\ref{eq:xi_eta_zeta_x_y_z}) 式からも得られる．したがって，sidereal 座標系でのJacobi積分の表式は，
\begin{equation}
C_J = 2 \left( \frac{\mu_1}{r_1} + \frac{\mu_2}{r_2} \right) + 2 n (\dot{\xi} \eta - \dot{\eta} \xi) - \dot{\xi}^2 - \dot{\eta}^2 - \dot{\zeta}^2
\end{equation}
となる．これは以下のように書き直せる．
\begin{equation}
